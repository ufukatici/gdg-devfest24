                    ...
                    <div class="col-12">
                        <div class="card mb-30">
                            <div class="card-body pt-30">
                                <h4 class="font-20">Çekiliş Kayıtları</h4>
                                <div class="right-top col-6">
                                    <div class="input-group addon radius-50 ov-hidden">
                                        <input type="text" class="form-control style--two" placeholder="İsim Ara" id="searchInput">
...
document.addEventListener("DOMContentLoaded", function () {
    const searchInput = document.querySelector("input[placeholder='İsim Ara']");
    const tableBody = document.querySelector("tbody");

    function fetchData(query = "") {
        fetch(`includes/getRecords.php?q=${query}`)
    ...
    function renderTable(records) {
        ...
        records.forEach((record, index) => {
            ...
            row.innerHTML = `
                ...
                <td>
                    <label>
                        <input type="checkbox" class="status-checkbox" data-id="${record.id}" ${record.status == 1 ? "checked" : ""}>
                    </label>
                </td>
                <td>
                    <label>
                        <input type="checkbox" class="success-checkbox" data-id="${record.id}" ${record.success == 1 ? "checked" : ""}>
                    </label>
                </td>
            `;
    ...
    function addCheckboxListeners() {
        ...
        const successCheckboxes = document.querySelectorAll(".success-checkbox");

        statusCheckboxes.forEach((checkbox) => {
            checkbox.addEventListener("change", function () {
                updateRecord(this.dataset.id, "status", this.checked ? 1 : 0);
            });
        });

        successCheckboxes.forEach((checkbox) => {
            checkbox.addEventListener("change", function () {
                updateRecord(this.dataset.id, "success", this.checked ? 1 : 0);
            });
        });
    }

    function updateRecord(id, field, value) {
        fetch("includes/updateRecord.php", {
            method: "POST",
            headers: {
